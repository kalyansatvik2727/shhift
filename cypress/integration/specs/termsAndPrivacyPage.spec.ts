...
describe('Shift | Terms and Privacy page',()=>{
    it('Terms and Privacy page validations',()=>{
        ...
        //Terms link
        cy.get('aside li a',{timeout:100000}).eq(1).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Terms')
        })
        cy.get('aside li a',{timeout:100000}).eq(1).click()
        cy.get('p strong',{timeout:100000}).eq(0).invoke('text').then((text)=>{
        ...
        //Privacy Policy link
        cy.get('aside li a',{timeout:100000}).eq(2).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Privacy Policy')
        })
        cy.get('aside li a',{timeout:100000}).eq(2).click()
        cy.get('div[id=privacy-policy] div[class=category-items] div p',{timeout:100000}).eq(0).invoke('text').then((text)=>{
            cy.wrap(text).should('contain','Financial Privacy Policy')
        })

        //Financial Privacy Policy link
        cy.get('aside li a',{timeout:100000}).eq(3).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Financial Privacy Policy')
        })
        cy.get('aside li a',{timeout:100000}).eq(3).click()
        cy.get('div[id=financial-privacy-policy] div[class=category-items] div p',{timeout:100000}).eq(1).invoke('text').then((text)=>{
        ...
        cy.get('aside li a',{timeout:100000}).eq(4).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Applicant Privacy Policy')
        })
        cy.get('aside li a',{timeout:100000}).eq(4).click()
        cy.get('div[id=applicant-privacy-policy] div[class=category-items] div p a',{timeout:100000}).eq(0).invoke('text').then((text)=>{
