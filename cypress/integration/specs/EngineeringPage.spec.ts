...
describe('Shift | Engineering page',()=>{
    it('Engineering page validations',()=>{
        ...
        cy.get('div[class*=ContentToggle__link]',{timeout:100000}).eq(0).click()
        cy.get('.ContentToggle__content',{timeout:100000}).eq(0).invoke('text').then((text)=>{
        ...
        cy.get('a',{timeout:100000}).contains('A self-driven purchase process').click()

        //modal validation        
        cy.get('div[class*=Modal__content] h2',{timeout:100000}).invoke('text').then((text)=>{
            cy.wrap(text).should('contain','A self-driven purchase process')
        })
        cy.get('.Modal__close').click({force:true})

        //Logistics platforms
        cy.get('div[class*=ContentToggle__link]',{timeout:100000}).eq(1).click()
        cy.get('.ContentToggle__content',{timeout:100000}).eq(1).invoke('text').then((text)=>{
        ...
        cy.get('a',{timeout:100000}).contains('Efficient field staffing').click({force:true})
        ...
        cy.get('a',{timeout:100000}).contains('Extended workflow app').click({force:true})
        ...
        //Pricing engine
        cy.get('div[class*=ContentToggle__link]',{timeout:100000}).eq(2).click()
        cy.get('.ContentToggle__content',{timeout:100000}).eq(2).invoke('text').then((text)=>{
        ...
        cy.get('a',{timeout:100000}).contains('Targeted inventory & pricing').click({force:true})
        ...
        //Meet the Team text 
        cy.get('div p',{timeout:100000}).eq(2).invoke('text').then((text)=>{
        ...
        cy.get('a span',{timeout:100000}).eq(12).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Read about our experiences')
        })
        cy.get('a[target=_blank]').eq(7).should('have.attr','href','https://blog.shift.com/')

        //join us
        cy.get('div[class*=ContentWidth__root] h2',{timeout:100000}).eq(1).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Join us!')
        })

        //join us text
        cy.get('div[class*=ContentWidth__root] p',{timeout:100000}).eq(3).invoke('text').then((text)=>{
            cy.wrap(text).should('contain','We’ve accomplished a lot with a small and talented team, but there’s more to be done.')
        })

        cy.get('div',{timeout:100000}).contains('Engineering, Product, and Design').should('be.visible')
        cy.get('div',{timeout:100000}).contains('You work on exciting, meaningful products.').should('be.visible')
