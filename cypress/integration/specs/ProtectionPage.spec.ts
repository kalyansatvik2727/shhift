...
describe('Shift | Protection page',()=>{
    it('Protection page validations',()=>{
        ...
        cy.get('h1',{timeout:100000}).invoke('text').then((text)=>{
        ...
        cy.get('h4',{timeout:100000}).eq(1).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Protection against the unexpected')
        })

        //Key lockout header
        cy.get('h5',{timeout:100000}).eq(0).invoke('text').then((text)=>{
            cy.wrap(text).should('eq','Key lockout')
        })

        //Key lockout header text
        cy.get('li p',{timeout:100000}).eq(0).invoke('text').then((text)=>{
