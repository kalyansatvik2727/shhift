...
Given('I navigate to Protection Page with required login', () => {
...
Then('I validate Peace of mind now comes standard section', () => {
  ...
  cy.get('div p', { timeout: 72000 })
    ...
    .then((text) => {
      cy.wrap(text).should(
        'eq',
        'Safeguard yourself from the unexpected costs of car ownership with a vehicle protection plan and gap coverage'
      );
    });
});
...
And('I validate Vehicle Protection Plan section', () => {
  ...
  cy.get('div p', { timeout: 72000 })
    ...
    .then((text) => {
      cy.wrap(text).should(
        'eq',
        'Most warranties last for less than a third of a car’s lifetime when it’s least likely to have issues. Our protection plans provide comprehensive coverage against mechanical and electrical problems so you don’t have to pay out of pocket.'
      );
    });
});
...
And('I validate GAP COVERAGE section', () => {
  ...
  cy.get('div p', { timeout: 72000 })
    ...
    .then((text) => {
      cy.wrap(text).should(
        'eq',
        'Gap coverage covers the “gap” between what your auto insurance pays and what you still owe on the car if you ever experience an accident or theft.'
      );
    });
});
...
And('I validate Gap Coverage section', () => {
  //Gap Coverage header validation
  cy.get('div[class=WarrantyPage__gap-content] >div > div:nth-child(1)', {
    timeout: 72000,
  })
    .eq(0)
    .invoke('text')
  ...
  //Gap Coverage header validation
  cy.get('div[class=WarrantyPage__gap-content] >div > div:nth-child(2)', {
    timeout: 72000,
  })
    .eq(0)
    .invoke('text')
