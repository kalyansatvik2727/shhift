...
When('I validate About landing page text', () => {
  ...
  cy.get('p div', { timeout: 72000 })
    ...
    .then((text) => {
      ...
      cy.wrap(text).should(
        'eq',
        'We are building the platform to make buying and selling cars fun, fair, and accessible to everyone.'
      );
    });
});
...
And('I validate Executive team section', () => {
  ...
  cy.get('h3[class*=Typo]', { timeout: 72000 })
    .eq(0)
    .invoke('text')
    .then((text) => {
  ...
  cy.get('.ProfileGrid__team-profile span')
    .eq(0)
    .invoke('text')
    .then((text) => {
  ...
  //validate the dialog
  cy.get('.ProfileGrid__team-profile div > img')
    .eq(1)
    .should('have.attr', 'alt', 'George Arison');
  //close the dialog
  ...
  cy.get('.ProfileGrid__team-profile div > img')
    .eq(2)
    .should('have.attr', 'alt', 'Toby Russell');
  //close the dialog
  ...
  cy.get('.ProfileGrid__team-profile div > img')
    .eq(4)
    .should('have.attr', 'alt', 'Cindy Hanford');
  //close the dialog
  cy.get('#Explorations').click({ force: true });
  ...
  cy.get('.ProfileGrid__team-profile div > img')
    .eq(8)
    .should('have.attr', 'alt', 'Christian Ohler');
  //close the dialog
  cy.get('#Explorations').click({ force: true });
  ...
  cy.get('.ProfileGrid__team-profile div > img')
    .eq(10)
    .should('have.attr', 'alt', 'Karan M. Gupta');
  //close the dialog
  cy.get('#Explorations').click({ force: true });
  ...
  cy.get('.ProfileGrid__team-profile div > img')
    .eq(12)
    .should('have.attr', 'alt', 'Tracy Lessin');
  //close the dialog
  cy.get('#Explorations').click({ force: true });
  ...
  cy.get('.ProfileGrid__team-profile div > img')
    .eq(14)
    .should('have.attr', 'alt', 'Josh Pittman');
  //close the dialog
  cy.get('#Explorations').click({ force: true });
...
And('I validate Latest Pre Releases section', () => {
  ...
  cy.get('div[id*=latest-press-releases] a[href*=essential]')
    .should('be.visible')
    .invoke('text')
    .then((text) => {
  ...
  cy.get('div[id*=latest-press-releases] a[href*=covid]')
    .should('be.visible')
    .invoke('text')
    .then((text) => {
  ...
  cy.get('div[id*=latest-press-releases] a[href*=shift-launches-digital]')
    .should('be.visible')
    .invoke('text')
    .then((text) => {
  ...
  cy.get('div[id*=latest-press-releases] a[href*=repairs]')
    .should('be.visible')
    .invoke('text')
    .then((text) => {
  ...
  cy.get('div[id*=latest-press-releases] a[href*=finance]')
    .should('be.visible')
    .invoke('text')
    .then((text) => {
