...
Then('I validate Engineering at Shift section', () => {
  ...
  cy.get('section > div > div > div > div > div', { timeout: 50000 })
    .eq(0)
    .invoke('text')
    .then((text) => {
  ...
  cy.get('section > div > div > div > div:nth-child(2) > div', {
    timeout: 50000,
  })
    .eq(0)
    .invoke('text')
    .then((text) => {
      cy.wrap(text).should(
        'eq',
        'At Shift, we empower engineers to solve high-impact problems that drive our business. Our platform serves both customers and our operational team, providing a plethora of engineering challenges.'
      );
    });

  //join the team button
  cy.get('button span', { timeout: 50000 })
    .invoke('text')
    .then((text) => {
...
And('I validate Marketplace & consumer products section', () => {
  ...
  cy.get('.ContentToggle__content', { timeout: 50000 })
    .eq(0)
    .invoke('text')
    .then((text) => {
      cy.wrap(text).should(
        'contain',
        'Buying and selling a car are complex transactions with many moving parts.'
      );
    });

  //A self-driven purchase process link
  cy.get('a', { timeout: 50000 })
    .contains('A self-driven purchase process')
    .click();

  //modal validation
  cy.get('div[class*=Modal__content] h2', { timeout: 50000 })
    .invoke('text')
    .then((text) => {
...
And('I validate Logistics platforms section', () => {
  ...
  cy.get('.ContentToggle__content', { timeout: 50000 })
    ...
    .then((text) => {
      cy.wrap(text).should(
        'contain',
        'It’s a classic computer science problem: how can we create the optimal schedule for drivers AND customers'
      );
    });
  //Efficient field staffing link
  cy.get('a', { timeout: 50000 })
    .contains('Efficient field staffing')
    .click({ force: true });
  ...
  cy.get('a', { timeout: 50000 })
    .contains('Extended workflow app')
    .click({ force: true });
...
And('I validate Pricing engine section', () => {
  ...
  cy.get('.ContentToggle__content', { timeout: 50000 })
    ...
    .then((text) => {
      cy.wrap(text).should(
        'contain',
        'Because no two cars are the same, pricing is a huge algorithmic challenge that’s at the core of our business.'
      );
    });
  //Targeted inventory & pricing link
  cy.get('a', { timeout: 50000 })
    .contains('Targeted inventory & pricing')
    .click({ force: true });
...
And('I validate our stack section', () => {
  ...
  //our stack text
  cy.get('section div p', { timeout: 50000 })
    .eq(0)
    .invoke('text')
    .then((text) => {
      cy.wrap(text).should(
...
And('I validate Meet the Team section', () => {
  //Meet the Team
  cy.get('h4[class*=Typo]', { timeout: 50000 })
    .eq(0)
    .invoke('text')
    .then((text) => {
  ...
  //Meet the Team text
  cy.get('div > div > p', { timeout: 50000 })
    .eq(1)
    .invoke('text')
    .then((text) => {
      cy.wrap(text).should(
...
And('I validate Read about our experiences section', () => {
  ...
  cy.get('div > a > span', { timeout: 50000 })
    .eq(0)
    .invoke('text')
    .then((text) => {
  ...
  cy.get('a[target=_blank]')
    .eq(7)
    .should('have.attr', 'href', 'https://blog.shift.com/');
});

And('I validate join us text section', () => {
  //join us
  cy.get('div[class*=ContentWidth__root] h2', { timeout: 50000 })
    .eq(1)
    .invoke('text')
    .then((text) => {
  ...
  cy.get('section div p', { timeout: 50000 })
    ...
    .then((text) => {
      cy.wrap(text).should(
        'contain',
        'We’ve accomplished a lot with a small and talented team, but there’s more to be done.'
      );
    });

  cy.get('div', { timeout: 50000 })
    .contains('Engineering, Product, and Design')
    .should('be.visible');
  cy.get('div', { timeout: 50000 })
    .contains('You work on exciting, meaningful products.')
    .should('be.visible');
});
