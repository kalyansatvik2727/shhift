...
And('I validate Whats included in Shifts Certification? section', () => {
    ...
    cy.get('h2', { timeout: 72000 }).eq(0).invoke('text').then((text) => {
...
And('I validate Not every car becomes Shift Certified section', () => {
    ...
    cy.get('h2', { timeout: 72000 }).eq(1).invoke('text').then((text) => {
...
And('I validate Total transparency section', () => {
    //Total transparency
    cy.get('h2', { timeout: 72000 }).eq(2).invoke('text').then((text) => {
        cy.wrap(text).should('eq', 'Total transparency.')
    })
    cy.get('div p', { timeout: 72000 }).eq(3).invoke('text').then((text) => {
...
And('I validate Love it or bring it back: our 7-day guarantee section', () => {
    ...
    cy.get('h2', { timeout: 72000 }).eq(3).invoke('text').then((text) => {
        cy.wrap(text).should('eq', 'Love it or bring it back: our 7-day guarantee')
    })
    cy.get('div p', { timeout: 72000 }).eq(4).invoke('text').then((text) => {
...
And('I validate Free 30-day powertrain warranty included section', () => {
    ...
    cy.get('h2', { timeout: 72000 }).eq(4).invoke('text').then((text) => {
        cy.wrap(text).should('eq', 'Free 30-day powertrain warranty included')
    })
    cy.get('div p', { timeout: 72000 }).eq(5).invoke('text').then((text) => {
...
And('I validate Extended coverage available section', () => {
    ...
    cy.get('h2', { timeout: 72000 }).eq(5).invoke('text').then((text) => {
        cy.wrap(text).should('eq', 'Extended coverage available')
    })
    cy.get('div p', { timeout: 72000 }).eq(6).invoke('text').then((text) => {
...
And('I validate Meet our trusted mechanics section', () => {
    ...
    cy.get('h2', { timeout: 72000 }).eq(6).invoke('text').then((text) => {
        cy.wrap(text).should('eq', 'Meet our trusted mechanics')
    })
    cy.get('div p', { timeout: 72000 }).eq(7).invoke('text').then((text) => {
    ...
    cy.get('h2', { timeout: 72000 }).eq(7).invoke('text').then((text) => {
        cy.wrap(text).should('eq', 'Find the car your mechanic would buy')
    })

    cy.get('button span', { timeout: 72000 }).invoke('text').then((text) => {
