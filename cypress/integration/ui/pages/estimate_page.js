class EstimatePage {
	...
	verifyOOAPage(){
		...
		cy.get('div', {timeout:10000}).contains('Thanks for your interest in Shift! Unfortunately, we don’t operate in your region yet.');
	}

	verifyEstimatePageTesla(){
		cy.url().should('include', 'escalated');
		cy.get('div', {timeout:10000}).contains('Thanks for requesting an estimate');
		cy.get('div', {timeout:10000}).contains('Thank you for requesting an estimate with us! Tesla vehicles are uniquely built, and as an owner, only you can access the unique build specifications required for us to price your vehicle accurately.');
		cy.get('div', {timeout:10000}).contains('If you provide your contact information below, we will send you a detailed follow-up email with the simple steps required for you to retrieve your Tesla Order Agreement. Once we receive those details, one of our pricing specialists will generate an estimate for your vehicle and contact you to discuss next steps.');
